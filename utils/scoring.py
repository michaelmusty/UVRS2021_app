...
def build_filtered_df(participants: List[Participant], N: int):
    ...
    # each pair (p,race) corresponds to a row of df
    for p in participants:
        col_i: List[str] = []  # these are partial columns for rows corresponding to p only
        col_s: List[int] = []
        col_r: List[str] = []
        col_a: List[str] = []
        for race in p.races:
        ...
        while len(col_s) > N:
            min_ind = col_s.index(min(col_s))  # the index of the (first) minimum score in the list col_s
            col_i.pop(min_ind)
            col_s.pop(min_ind)
            col_r.pop(min_ind)
            col_a.pop(min_ind)
