...
class Participant:
    ...
    def __init__(
        ...
        *,
        ...
        racers: List[Racer],  # list of racers corresponding to the person in each race that person ran
    ):
        self.person = person
        if len(races) > 0:
            self.races = races
        else:
            raise Exception(f"a participant cannot be created unless number of races participated in is > 0")
        if len(racers) > 0:
            assert len(racers) == len(races)
            self.racers = racers
        else:
            raise Exception(f"a participant cannot be created unless number of of corresponding racers is > 0")
...
def compute_age_group(date_of_birth: date, race_date: date, gender: str) -> str:
    ...
    age = race_date.year - date_of_birth.year - ((race_date.month, race_date.day) < (date_of_birth.month, date_of_birth.day))
    assert age > 0
    if age <= 29:
        return f"{gender}029"
    elif age <= 39:
        ...
        raise Exception(f"is there really someone older than 120 year in the race results?")